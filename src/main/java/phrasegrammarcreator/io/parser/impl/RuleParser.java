...
public class RuleParser extends SingleValueParser<Rule> {
    ...
    @Override
    public Rule parse(String rule) throws Exception {
        ...
        Phrase to = phraseParser.parse(matcher.group(2));

        if (from.size() == 1)
            return new ContextFreeRule((NonTerminal) from.get(0).getBuilder(), to);
        else
            return new ContextSensitiveRule(from, to);
    }
}
